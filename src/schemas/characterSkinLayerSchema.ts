...
export const CharacterSkinLayerSchema = {
    ...
    masks: { type: 'schema', show: { type: ['static'] }, tooltip: 'Clip all image layers below this layer inside this mask boundaries.' },
    ...
    art_dx: { type: 'number', show: { type: ['static'] }, tooltip: 'X shift in % for this layer\'s art positioning. Falls back to character art_dx trait when unset.' },
    art_dy: { type: 'number', show: { type: ['static'] }, tooltip: 'Y shift in % for this layer\'s art positioning. Falls back to character art_dy trait when unset.' },
    art_scale: { type: 'number', show: { type: ['static'] }, tooltip: 'Scale multiplier for this layer\'s art. Falls back to character art_scale trait when unset.' },
