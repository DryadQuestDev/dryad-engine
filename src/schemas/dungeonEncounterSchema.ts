...
export const DungeonEncounterSchema = {
  ...
  collect_item: { type: 'chooseOne', fromFile: 'item_templates', show: { type: ['collectable'] }, tooltip: 'Item granted by the Collect choice. Needs no content document entry — the description falls back to the item\'s name and description. Write an @ line with the same id to override the prose or add if/discover gating; inside it, |title| and |description| substitute the granted item\'s name and description.' },
  collect_pool: { type: 'string', show: { type: ['collectable'] }, tooltip: 'Random gather spot: name of a gather table (e.g. "gather"). The item is drawn once at dungeon creation through the collectable_resolve emitter — the experience plugin answers "gather" with a tier-weighted roll filtered by the dungeon\'s environment trait — and stays locked for the save. Takes precedence over collect_item; leave the image empty to use the drawn item\'s icon.' },
