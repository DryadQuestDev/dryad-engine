...
const gdocBlockCollapsed = useStorage('editorDocument_gdocBlockCollapsed', false)
...
<template>
  <div class="editor-document">
    ...
    <div> <!-- v-if="isElectronEnvironment"-->
      ...
      <div class="gdoc-block" :class="{ 'gdoc-block--collapsed': gdocBlockCollapsed }">
        <div class="gdoc-block-header" @click="gdocBlockCollapsed = !gdocBlockCollapsed">
          <i class="pi pi-google gdoc-block-icon" />
          <span class="gdoc-block-title">Google Docs Integration</span>
          <span class="gdoc-block-spacer"></span>
          <i :class="['pi', gdocBlockCollapsed ? 'pi-chevron-down' : 'pi-chevron-up', 'gdoc-block-chevron']" />
        </div>

        <div v-if="!gdocBlockCollapsed" class="gdoc-block-body">
          <!-- Error and Loading Messages -->
          <div v-if="documentManager.isLoading.value && !documentManager.isOAuthServerWaitingForCallback.value"
            class="loading-message">Loading...</div>
          ...
          <Accordion :value="[]">
            <AccordionPanel value="google-document-integration" style="border: none;">
              <AccordionHeader>
                <h2>Manage Google API</h2>
              </AccordionHeader>
              <AccordionContent>
                <!-- Main Tab Navigation -->
                <div class="main-tabs-nav">
                  <Button label="OAuth Apps" @click="activeTab = 'setupCredentials'"
                    :text="activeTab !== 'setupCredentials'"
                    :class="{ 'p-button-text': activeTab !== 'setupCredentials' }"></Button>
                  <Button label="Create New Token" @click="activeTab = 'createToken'"
                    :text="activeTab !== 'createToken'"
                    :class="{ 'p-button-text': activeTab !== 'createToken' }"></Button>
                  <Button label="Add Existing Token" @click="activeTab = 'addToken'" :text="activeTab !== 'addToken'"
                    :class="{ 'p-button-text': activeTab !== 'addToken' }"></Button>
                  <Button label="User Tokens" @click="activeTab = 'listTokens'" :text="activeTab !== 'listTokens'"
                    :class="{ 'p-button-text': activeTab !== 'listTokens' }"></Button>
                ...
                <div class="tab-content-area">
                  ...
                  <div v-if="activeTab === 'setupCredentials'" class="config-section credentials_popup">
                    ...
                    <p>
                      ...
                      <a href="https://console.cloud.google.com/" target="_blank" rel="noopener noreferrer">Google Cloud
                        Console</a>.
                    </p>
                    <textarea v-model="newClientSecretJsonInput" rows="5"
                      placeholder="Paste content of your client_secret.json here"></textarea>
                    <input type="text" v-model="newOAuthAppNameInput"
                      placeholder="Optional: Give this OAuth App a name (e.g., 'My Project')" />
                    ...
                    <div v-if="availableOAuthApps.length > 0" class="oauth-app-list">
                      ...
                      <ul>
                        <li v-for="appConfig in availableOAuthApps" :key="appConfig.clientId"
                          :class="{ 'active-selection': appConfig.clientId === selectedOAuthAppIdForNewToken }">
                          <strong>{{ appConfig.appName || appConfig.clientId }}</strong> (Client ID: {{
                            appConfig.clientId }})
                          <Button
                          ...
                          <Button icon="pi pi-trash" @click="deleteOAuthApp(appConfig.clientId)" severity="danger"
                            class="p-button-sm p-button-outlined" aria-label="Delete OAuth App" />
                        </li>
                      </ul>
                    </div>
                  ...
                  <div v-if="activeTab === 'createToken'" class="config-section setup_token_popup">
                    ...
                    <div v-else>
                      <p>Using OAuth App: <strong>{{documentManager.oauthAppConfigs.find(a => a.clientId ===
                        selectedOAuthAppIdForNewToken)?.appName || selectedOAuthAppIdForNewToken }}</strong></p>
                      <input type="text" v-model="newTokenNameInput"
                        placeholder="Enter a unique name for this token (e.g., 'My Main Account Token')" />
                      <Button
                      ...
                      <p class="small-text"
                        v-if="documentManager.isOAuthServerWaitingForCallback.value && !documentManager.pendingAuthUrl.value">
                        Authentication in progress. Click abort to stop and cancel.
                      </p>
                      <div v-if="documentManager.pendingAuthUrl.value" class="auth-link-inline"
                        style="margin-top: 8px;">
                        <a :href="documentManager.pendingAuthUrl.value" target="_blank" rel="noopener noreferrer"
                          class="auth-link">
                          Click here to authenticate with Google →
                        </a>
                        <p class="small-text" style="margin-top: 4px;">After authenticating, return here. Click abort to
                          cancel.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div v-if="activeTab === 'addToken'" class="config-section">
                    ...
                    <div>
                      <p v-if="selectedOAuthAppIdForNewToken">Token will be associated with OAuth App: <strong>{{
                        documentManager.oauthAppConfigs.find(a => a.clientId ===
                          selectedOAuthAppIdForNewToken)?.appName || selectedOAuthAppIdForNewToken}}</strong></p>
                      <p v-else class="small-text">No OAuth App is currently selected. The token will be added without a
                        specific OAuth App linkage (you can still use it directly).</p>
                      <input type="text" v-model="existingTokenNameInput"
                        placeholder="Enter a unique name for this token" />
                      <textarea v-model="existingTokenJsonInput" rows="3"
                        :placeholder="'Paste token data, e.g., { \'access_token\': \'value\', ... }'"></textarea>
                      <Button label="Add Existing Token" @click="addExistingUserToken"
                        :disabled="documentManager.isLoading.value" icon="pi pi-plus" />
                    </div>
                  </div>
                  ...
                  <div v-if="activeTab === 'listTokens'" class="config-section user-token-list">
                    ...
                    <div v-if="availableUserTokens.length > 0">
                      <ul>
                        <li v-for="token in availableUserTokens" :key="token.id" :class="{
                          ...
                          <div class="token-actions">
                            <Button @click="setTokenAsGlobalDefault(token.id)"
                            ...
                            <Button icon="pi pi-eye" @click="toggleRevealToken(token.id)" title="Reveal/Hide Token JSON"
                              class="p-button-rounded p-button-text p-button-secondary" />
                            <span class="token-name" @click="setTokenForCurrentConfig(token.id)"
                              title="Click to set for current game/mod/dungeon config">{{ token.userGivenName }}</span>
                            <Button @click="deleteUserTokenEntry(token.id)" icon="pi pi-trash" severity="danger"
                              class="p-button-sm p-button-rounded p-button-text delete-icon"
                              aria-label="Delete Token" />
                          </div>
                          <div v-if="token.googleUser" class="token-profile-info">
                            <img v-if="token.googleUser.picture" :src="token.googleUser.picture" alt="User"
                              class="profile-pic-small" />
                            <span>{{ token.googleUser.name }}</span>
                          </div>
                          <div v-else class="info-message small-text token-profile-info">Google User profile not
                            available.</div>
                          <div class="token-associations">
                            <small>OAuth App:
                              <template v-if="token.linkedOAuthAppClientId">
                                {{documentManager.oauthAppConfigs.find(a => a.clientId ===
                                  token.linkedOAuthAppClientId)?.appName || token.linkedOAuthAppClientId.substring(0, 10)
                                }}...
                              </template>
                              <template v-else>
                            ...
                            <small v-if="token.id === documentManager.globalDefaultUserTokenId.value"
                              class="global-default-indicator"> (Global
                              Default)</small>
                            <small v-if="token.id === currentConfigSelectedTokenId" class="config-default-indicator">
                              (Set for
                              {{ currentConfigKey }})</small>
                          </div>
                          <div v-if="revealedTokenId === token.id" class="token-reveal-json">
                            <textarea readonly rows="8">{{ revealedTokenJson }}</textarea>
                            <Button label="Copy JSON" @click="copyToClipboard(revealedTokenJson)" icon="pi pi-copy"
                              class="p-button-sm" />
                          </div>
                        </li>
                      </ul>
                      <div>
                        <label for="user-token-select-import">Active Token for <strong>Import Document</strong>
                          tab:</label>
                        <select id="user-token-select-import" v-model="selectedUserTokenIdForImport"
                          @change="selectActiveUserTokenForImport">
                        ...
                        <span v-if="selectedUserTokenIdForImport" class="active-indicator">✓ Selected for fetching the
                          document</span>
                      </div>
                    </div>
                    <div v-else class="info-message">No user tokens found. Please create one or add an existing token.
                    </div>
                  </div>
...
-->
                ...
                <div class="config-section clear-all-section">
                  <Button label="Clear ALL OAuth Apps & User Tokens" @click="clearAllGoogleData"
                    v-tooltip.right="global.getString('msg.clear_all')" :disabled="documentManager.isLoading.value"
                    severity="danger" icon="pi pi-exclamation-triangle" />
                </div>
              </AccordionContent>
            </AccordionPanel>
          ...
          <div class="fetch-document-section">
            ...
            <div v-if="global.storageService.supportsGoogleAuth() && !selectedUserTokenIdForImport"
              class="warn-message">
              Please select an active User Token from the "User Tokens" tab (via dropdown or by activating one) to use
              for
              fetching.
            </div>
            <template v-else>
              <Button raised label="Fetch Google Document" @click="fetchDoc" :disabled="documentManager.isLoading.value"
                icon="pi pi-download" />
...
.gdoc-block-header:hover {
  background: rgba(66, 133, 244, 0.16);
}

.gdoc-block--collapsed .gdoc-block-header {
  border-bottom: none;
}
...
.main-tabs-nav {
  ...
  /* Replaced by Button group styling */
  margin-bottom: 1rem;
  gap: 3px;
}
...
.main-tabs-nav .p-button {
  ...
  /* For non-active PrimeVue text buttons */
}
